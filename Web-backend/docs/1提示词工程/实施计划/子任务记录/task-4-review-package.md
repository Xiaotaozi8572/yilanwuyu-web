...
from input.query_understanding import understand_query
from prompts.runtime import PromptRuntime
from services.model_client import MockModelClient, ModelOptions
...
from core.actions import ActionDecision
from core.contracts import RunTrace, SceneState, validate_contract
from core.errors import ContractValidationError


def test_action_decision_contains_required_actions():
    assert {action.value for action in ActionDecision} == {
        "PASS",
        "REWRITE_ONLY",
        "RETRIEVE_MORE",
        "ASK_CLARIFICATION",
...
def test_run_trace_minimal_fields_validate():
    trace = validate_contract(
        {
            "run_id": "run_test",
            "current_state": "INPUT_RECEIVED",
    ...
    assert trace.prompt_snapshot_id is None
    assert trace.prompt_route_reason is None
    assert trace.prompt_missing_variables == []
    assert trace.prompt_injection_summary == []


def test_contract_validation_error_contains_field_path():
    with pytest.raises(ContractValidationError, match="run_id"):
        RunTrace(run_id="")


def test_scene_state_contract_can_represent_selected_object():
    scene = SceneState(
        aircraft_id="c919",
        component_id="wing",
    ...
    assert scene.to_dict()["selected_object_id"] == "scene_wing_001"
```
