...
Full current contents after the Task 1 review fix.
...
import prompts.asset_models as asset_models
...
def test_prompt_domain_models_live_in_prompts_asset_models_and_prompt_asset_is_not_a_core_contract():
    assert "prompt_asset" not in contracts.CONTRACT_TYPES
    assert asset_models.TeachingPromptAsset.__module__ == "prompts.asset_models"
    assert asset_models.PromptSelection.__module__ == "prompts.asset_models"
    assert asset_models.PromptMessageBundle.__module__ == "prompts.asset_models"
