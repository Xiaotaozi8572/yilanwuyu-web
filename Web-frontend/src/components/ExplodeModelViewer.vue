<template>
  <div ref="host" class="explode-model-viewer" :class="{ 'is-ready': isReady }">
    ...
    <div v-if="!isReady && !loadError" class="model-loading" role="status">
      ...
      <span>正在装载 C919 拆解模型 {{ loadProgress }}%</span>
    </div>
...
onMounted(() => {
  ...
  renderer.domElement.setAttribute('aria-label', 'C919 爆炸拆解三维模型')
  ...
  Promise.all([
    ...
    .then(([explodedGltf, assembledGltf]) => {
      ...
      const explodedBounds = new THREE.Box3().setFromObject(explodedRoot)
      ...
      const distance = (displayRadius / Math.sin(fitFov / 2)) * 0.48
