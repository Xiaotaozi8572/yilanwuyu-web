<template>
  <SceneShell :image="sceneImage" alt="C919 知识巩固测试" image-blend-mode="screen">
    <template #background>
    ...
    <button class="scene-hotspot back-hotspot" @click="$router.push('/interaction')">
...
const questionBank = [
  { image: 1, answer: 2, type: 'choice' },
  { image: 2, answer: 1, type: 'judge' },
  { image: 3, answer: 1, type: 'choice' },
  { image: 4, answer: 0, type: 'judge' },
  { image: 5, answer: 1, type: 'choice' },
  { image: 6, answer: 1, type: 'judge' },
  { image: 7, answer: 1, type: 'choice' },
  { image: 8, answer: 0, type: 'judge' }
]
...
const sceneImage = computed(() => `8学习页/${activeQuestion.value.image}.jpg`)
const optionHotspots = computed(() => optionLayouts[questions[currentIndex.value].type])
const answerLetter = computed(() => {
  const question = questions[currentIndex.value]
  if (question.type === 'judge') return question.answer === 0 ? '正确' : '错误'
...
function advance() {
  ...
  router.push({ path: '/result', query: { score: score.value, total: questions.length } })
}
