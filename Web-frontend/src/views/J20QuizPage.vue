<template>
  <SceneShell asset-base="/j20-assets/" :image="sceneImage" alt="歼20 知识巩固测试" image-blend-mode="screen">
    <template #background>
...
const questionBank = [
  { image: '歼20.jpg', answer: 1, type: 'choice' },
  { image: '歼201.jpg', answer: 1, type: 'choice' },
  { image: '歼20-1.jpg', answer: 0, type: 'judge' },
  { image: '歼20-2.jpg', answer: 1, type: 'choice' },
  { image: '歼20-3.jpg', answer: 1, type: 'judge' },
  { image: '歼20-4.jpg', answer: 1, type: 'choice' },
  { image: '歼20-5.jpg', answer: 0, type: 'judge' },
  { image: '歼20-6.jpg', answer: 0, type: 'judge' }
]
...
function createQuestionOrder(items) {
  const previousFirstImage = sessionStorage.getItem('j20-quiz-first-image')
  const shuffled = shuffleQuestions(items)
  if (items.length > 1 && shuffled[0].image === previousFirstImage) {
    const replacementIndex = shuffled.findIndex((q) => q.image !== previousFirstImage)
    ;[shuffled[0], shuffled[replacementIndex]] = [shuffled[replacementIndex], shuffled[0]]
  }
  sessionStorage.setItem('j20-quiz-first-image', shuffled[0].image)
  return shuffled
}
...
const preserverUrl = computed(() => j20AssetPath(sceneImage.value))
...
function advance() {
  ...
  router.push({ path: '/j20/result', query: { score: score.value, total: questions.length } })
}
