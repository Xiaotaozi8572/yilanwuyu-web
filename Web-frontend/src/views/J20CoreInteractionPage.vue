<template>
  <SceneShell asset-base="/j20-assets/"
    class="core-interaction-scene"
    image="核心交互页/部件按钮点击.jpg"
    alt="歼20 核心交互页"
  >
    <template #background>
...
const partLabelHotspots = [
  {
    ...
    part: 'canard',
    label: '鸭式气动布局',
    style: { '--x': 19.17, '--y': 32.26, '--w': 10.9, '--h': 4.49 }
  },
  {
    key: 'inlet-label',
    part: 'inlet',
    label: '蚌式进气道',
    style: { '--x': 38.75, '--y': 55.24, '--w': 9.79, '--h': 4.49 }
  },
  {
    key: 'engine-label',
    part: 'engine',
    label: '发动机',
    style: { '--x': 63.33, '--y': 29.06, '--w': 7.57, '--h': 4.49 }
  },
  {
    key: 'tail-label',
    part: 'tail',
    label: '全动双垂尾',
    style: { '--x': 70.42, '--y': 42.41, '--w': 9.79, '--h': 4.49 }
  }
]
...
const partNodeHotspots = [
  {
    ...
    part: 'canard',
    label: '鸭式气动布局',
    style: { '--x': 34.51, '--y': 38.67, '--w': 1.55, '--h': 2.35 }
  },
  {
    key: 'inlet-node',
    part: 'inlet',
    label: '蚌式进气道',
    style: { '--x': 32.85, '--y': 50.64, '--w': 1.55, '--h': 2.35 }
  },
  {
    key: 'engine-node',
    part: 'engine',
    label: '发动机',
    style: { '--x': 57.92, '--y': 36.0, '--w': 1.55, '--h': 2.35 }
  },
  {
    key: 'tail-node',
    part: 'tail',
    label: '全动双垂尾',
    style: { '--x': 65.0, '--y': 37.18, '--w': 1.55, '--h': 2.35 }
  }
]

const menuHotspots = [
  { label: '爆炸拆解', route: '/j20/explode', style: { '--x': 83.819, '--y': 43.697, '--w': 7.361, '--h': 11.592 } },
  { label: '模拟驾驶', route: '/j20/cockpit', style: { '--x': 83.819, '--y': 55.288, '--w': 7.361, '--h': 11.592 } },
  { label: '虚拟课堂', route: '/j20/classroom', style: { '--x': 83.819, '--y': 66.88, '--w': 7.361, '--h': 11.592 } },
  { label: '知识巩固', route: '/j20/quiz', style: { '--x': 83.819, '--y': 78.472, '--w': 7.361, '--h': 11.592 } }
]
...
function openPart(part) {
  router.push({ path: '/j20/parts', query: { part } })
}
</script>
...
.core-interaction-scene :deep(.scene-vignette) {
  display: none;
}
