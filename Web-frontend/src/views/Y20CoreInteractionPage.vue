<template>
  <SceneShell asset-base="/y20-assets/"
    class="core-interaction-scene"
    image="核心交互页/部件点击.jpg"
    alt="运20 核心交互页"
  >
    <template #background>
    ...
    <button
      v-for="part in partLabelHotspots"
      :key="part.key"
      class="scene-hotspot part-label-hotspot"
      :data-label="part.label"
      :style="part.style"
      @click="openPart(part.part)"
    >
      <span class="sr-only">查看{{ part.label }}</span>
    </button>

    <button
      v-for="part in partNodeHotspots"
      :key="part.key"
      class="scene-hotspot part-node-hotspot"
      :data-label="part.label"
      :style="part.style"
      @click="openPart(part.part)"
    >
...
const partLabelHotspots = [
  {
    key: 'box-label',
    part: 'box',
    label: '外置中央翼盒',
    style: { '--x': 23.54, '--y': 33.65, '--w': 14.44, '--h': 4.49 }
  },
  {
    key: 'nose-label',
    part: 'nose',
    label: '圆润无下巴机头',
    style: { '--x': 12.57, '--y': 45.41, '--w': 13.33, '--h': 4.49 }
  },
  {
    key: 'engine-label',
    part: 'engine',
    label: '发动机',
    style: { '--x': 33.89, '--y': 56.2, '--w': 7.57, '--h': 4.49 }
  },
  {
    key: 'tail-label',
    part: 'tail',
    label: 'T型高平尾',
    style: { '--x': 63.89, '--y': 35.15, '--w': 9.38, '--h': 4.49 }
  },
  {
    key: 'wing-label',
    part: 'wing',
    label: '超临界机翼',
    style: { '--x': 60.56, '--y': 50.43, '--w': 9.79, '--h': 4.49 }
  }
]

const partNodeHotspots = [
  {
    key: 'box-node',
    part: 'box',
    label: '外置中央翼盒',
    style: { '--x': 42.36, '--y': 39.58, '--w': 1.55, '--h': 2.3 }
  },
  {
    key: 'nose-node',
    part: 'nose',
    label: '圆润无下巴机头',
    style: { '--x': 29.79, '--y': 46.9, '--w': 1.55, '--h': 2.3 }
  },
  {
    key: 'engine-node',
    part: 'engine',
    label: '发动机',
    style: { '--x': 45.54, '--y': 56.62, '--w': 1.55, '--h': 2.85 }
  },
  {
    key: 'tail-node',
    part: 'tail',
    label: 'T型高平尾',
    style: { '--x': 57.92, '--y': 36.48, '--w': 1.55, '--h': 2.3 }
  },
  {
    key: 'wing-node',
    part: 'wing',
    label: '超临界机翼',
    style: { '--x': 54.37, '--y': 51.92, '--w': 1.55, '--h': 2.35 }
  }
]

const menuHotspots = [
  { label: '爆炸拆解', route: '/y20/explode', style: { '--x': 83.819, '--y': 43.697, '--w': 7.361, '--h': 11.592 } },
  { label: '模拟驾驶', route: '/y20/cockpit', style: { '--x': 83.819, '--y': 55.288, '--w': 7.361, '--h': 11.592 } },
  { label: '虚拟课堂', route: '/y20/classroom', style: { '--x': 83.819, '--y': 66.88, '--w': 7.361, '--h': 11.592 } },
  { label: '知识巩固', route: '/y20/quiz', style: { '--x': 83.819, '--y': 78.472, '--w': 7.361, '--h': 11.592 } }
]
...
const timelineMilestones = [8.577, 23.868, 39.256, 55.135, 72.974, 90.225]
...
function openPart(part) {
  router.push({ path: '/y20/parts', query: { part } })
}
</script>
...
.part-label-hotspot::before {
  border-radius: 999px;
}

.part-label-hotspot:hover::before,
.part-label-hotspot:focus-visible::before {
  background: rgba(56, 232, 255, 0.08);
  box-shadow:
    inset 0 0 0 1px rgba(91, 235, 255, 0.78),
    0 0 18px rgba(56, 232, 255, 0.26);
}
...
.part-node-hotspot::before {
  border-radius: 999px;
}

.part-node-hotspot:hover::before,
.part-node-hotspot:focus-visible::before {
  background: rgba(56, 232, 255, 0.16);
  box-shadow:
    inset 0 0 0 2px rgba(255, 255, 255, 0.88),
    0 0 16px rgba(56, 232, 255, 0.58);
}
